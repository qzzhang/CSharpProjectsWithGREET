...
namespace FuzzyString
{
    ...
    { 
        public static bool ApproximatelyEquals(this string source, string target, List<FuzzyStringComparisonOptions> options, FuzzyStringComparisonTolerance tolerance)
        {
            ...
            {
                if (comparisonResults.Count > 0 && comparisonResults.Average() < 0.25)
                {
                    return true;
                }
                else
            ...
            {
                if (comparisonResults.Count > 0 && comparisonResults.Average() < 0.5)
                {
                    return true;
                }
                else
            ...
            {
                if (comparisonResults.Count > 0 && comparisonResults.Average() < 0.75)
                {
                    return true;
                }
                else
            ...
            {
                if (comparisonResults.Count > 0 && comparisonResults.Average() > 0.6)
                {
                    return true;
                }
                else
