...
{
    [Serializable]
    public static class MathParseWithReferences
    {
        ...
        /// <summary>
        /// Takes the display string and converts into a string that Model can evaluate.
        /// </summary>
        /// <param name="str"></param>
        /// <returns></returns>
        public static string ToInternalString(string str, GData data)
        {
            try
            {
                int startIndex = 0;
                ...
                while ((firstIndex = str.IndexOf("[", startIndex)) != -1)
                {
                    lastIndex = str.IndexOf("]", firstIndex);
                    string identifierstring = str.Substring(firstIndex + 1, lastIndex - firstIndex - 1);

                    str = str.Substring(0, firstIndex + 1) + data.ParametersData.Values.Single(item => item.Identifier == identifierstring).Id + str.Substring(lastIndex);
                    startIndex = firstIndex + 2;// advancing the index to look for the next "["
                }
            }
            catch
            { }
        ...
        /// <param name="str"></param>
        /// <returns></returns>
        public static KeyValuePair<uint, double> Parse(string str, GData data = null, Guid tracer = new Guid())
        {
            try
            {
                double res;
                ...
                if (MathParse.HasOps(ref str))
                {
                    Handle_Single_Cell_Values(data, ref str, tracer);
                    ...
                    if (str.Contains("$"))
                    {
                        string dim = MathParse.TakeOutGroup(ref str);
                        uint group = 0;
                        if(!string.IsNullOrEmpty(dim))
                            group = Convert.ToUInt32(dim);
                        Double.TryParse(str, System.Globalization.NumberStyles.Number, MathParse.Nfi, out res);
                        return new KeyValuePair<uint, double>(group, res);
                    }
                }

                if (Double.TryParse(str, System.Globalization.NumberStyles.Number, MathParse.Nfi, out res))
                    return new KeyValuePair<uint, double>(0, res);
                else
                    return new KeyValuePair<uint, double>(0, Double.NaN);
            }
            catch(Exception e)
            {
                LoggerLib.LogFile.Write(e.Message);
                throw new Exception("Formula is invalid or using non-existing parameters references");
                
            }
        }

        static Regex rhandle_single_cell = new Regex("\\[([A-Za-z0-9!_\\-\\(\\)]*)\\]");
        static char[] brackets = new char[] { '[', ']' };

        /// <summary>
        /// recursively convert all the table values to their real value
        /// then we have "THE BIG EQUATION" to convert with the math parser
        /// </summary>
        /// <param name="to_convert"></param>
        private static void Handle_Single_Cell_Values(GData data, ref string to_convert, Guid tracer)
        {
            
            string replacement_issues = "";
            int safety_int = 10000; //10.000 tries before giving up, this is then the max number of operands in a formula
            Match match;
            while((match = rhandle_single_cell.Match(to_convert)).Success && safety_int > 0)
            {
                safety_int--;
                string replacement_string = "";
                string valueMatched = match.Value;

                if (data == null)
                    throw new Exception("Can't handle reference to simulations parameters without a GData object reference");

                Parameter referencedParameter = null;
                InputTableObject value = data.InputsData.GetValue(valueMatched.Trim(brackets));
                if (value != null && value.Value is Parameter)
                    referencedParameter = value.Value as Parameter;
                else if (value != null && value.Value is ParameterTS)
                    referencedParameter = (value.Value as ParameterTS).CurrentValue;
                else if (value == null && data.ParametersData.ContainsKey(valueMatched.Trim(brackets)))
                    referencedParameter = data.ParametersData[valueMatched.Trim(brackets)];
                else if (value == null && data.ParametersData.Values.Any(item => item.Name == valueMatched.Trim(brackets)))
                    referencedParameter = data.ParametersData.Values.Single(item => item.Name == valueMatched.Trim(brackets));

                if (referencedParameter != null)
                {
                    if ((referencedParameter._greetFormulaString.Contains(valueMatched)
                        || referencedParameter._userFormulaString.Contains(valueMatched)))
                    {
                        replacement_string = "NaN";
                        replacement_issues += "Recursive call of the same value: " + valueMatched + "\r\n";
                    }
                    else if (referencedParameter != null)
                    {
                        referencedParameter.UpdateBuffers(data, tracer);
                        replacement_string = "(" + referencedParameter.ValueInDefaultUnit.ToStringFull() + "$" + referencedParameter.Dim + "$" + ")";
                    }
                    else
                        throw new Exception("Can't find this value: " + valueMatched);
                }
                else
                {
                    throw new InvalidParameterReferenceException(valueMatched + " is not a parameter in the database");
                }

                to_convert = to_convert.Replace(valueMatched, replacement_string);
            }
            if (String.IsNullOrEmpty(replacement_issues) == false)
            {
                //ToolsData.Messaging.Print(replacement_issues);
                to_convert = "NaN";
            }
        }


        private static void Handle_Paren(ref string str)
        {
            ...
            while (GetParenBlock(str, ref firstParen, ref lastParen))
            {
                ...
                {
                    KeyValuePair<uint, double> result = Parse(split[i]);
        ...
        private static void Handle_Scientific(ref string str)
        {
        ...
        private static void Handle_Functions(ref string str)
        {
            string function_name;
            bool unkown_funtion = false;
            Match match;
            while (str.Contains('{') && unkown_funtion == false)
            {
                match = rhandle_functions.Match(str);
        ...
        private static void Handle_Pow(ref string str)
        {
        ...
        private static void Handle_Div_Mult(ref string str)
        {
        ...
        private static void Handle_Add_Sub(ref string str)
        {
        ...
        private static void Handle_Booleans(ref string str)
        {
            int start = 0;          //Set substring start to beginning of str
            int end = str.Length - 1;   //Set substring end to end of str
        ...
        private static string Div(string x, string y)
        {
            ...
            uint dim_x = 0;
            uint.TryParse(group_x, out dim_x);
            uint dim_y = 0;
            uint.TryParse(group_y, out dim_y);

            uint newDim = DimensionUtils.Minus(dim_x, dim_y);

            return (Convert.ToDouble(x, Nfi) / Convert.ToDouble(y, Nfi)).ToStringFull() + "$" + newDim + "$";
        }
        private static string Mult(string x, string y)
        {
            ...
            uint newDim = DimensionUtils.Plus(dim_x, dim_y);
        ...
        private static string Add(string x, string y)
        {
            ...
            uint.TryParse(group_y, out dim_y);
            uint finalDim = 0;
            if (dim_y != DimensionUtils.RATIO && dim_x != DimensionUtils.RATIO && dim_y != dim_x)
                throw new InconsistantUnitsException("Can't add " + group_x + " and " + group_y);
            else
                finalDim = dim_x;

            return (Convert.ToDouble(x, Nfi) + Convert.ToDouble(y, Nfi)).ToStringFull() + "$" + finalDim + "$";
        }
        ...
        private static string TakeOutGroup(ref string str)
        {
        ...
        /// </summary>
        public static string formatStr = "0.#######################################################################################################";
        //the r used here prevents rounding
        //public  string formatStr = "r";
        #endregion parser
